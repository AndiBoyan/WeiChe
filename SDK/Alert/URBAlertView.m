...
#define kURBAlertButtonPadding 15.0
#define kURBAlertButtonHeight 40
#define kURBAlertButtonOffset 60
...
- (NSInteger)addButtonWithTitle:(NSString *)title {	
	...
	button.titleLabel.font = [UIFont fontWithName:self.titleFont.fontName size:14.0];
...
- (CGRect)defaultFrame {
	...
	CGRect insetFrame = CGRectIntegral(CGRectInset(appFrame, (appFrame.size.width - 240.0) / 2, (appFrame.size.height - 120.0) / 2));
...
- (void)animateWithType:(URBAlertAnimation)animation show:(BOOL)show completionBlock:(void (^)())completion {
	...
	// horizontal flip animation
        ...
        //CGFloat zAxis = (animation == URBAlertAnimationTumble) ? 1.0 : 0.0;
		...
		if (show) {
			...
			self.layer.transform = CATransform3DConcat(CATransform3DMakeRotation(70.0 * M_PI / 180.0, 1.0, 0.0, 0.0), perspectiveTransform);
			self.alpha = 0.0f;
			
			[self showOverlay:YES];
			
			[UIView animateWithDuration:0.2 animations:^{ // flip remaining + bounce
				self.layer.transform = CATransform3DConcat(CATransform3DMakeRotation(-25.0 * M_PI / 180.0, 1.0, 0.0, 0.0), perspectiveTransform);
				self.alpha = 1.0f;
			} completion:^(BOOL finished) {
				[UIView animateWithDuration:0.13 animations:^{
				 self.layer.transform = CATransform3DConcat(CATransform3DMakeRotation(12.0 * M_PI / 180.0, 1.0, 0.0, 0.0), perspectiveTransform);
				} completion:^(BOOL finished) {
					[UIView animateWithDuration:0.1 animations:^{
						self.layer.transform = CATransform3DConcat(CATransform3DMakeRotation(-8.0 * M_PI / 180.0, 1.0, 0.0, 0.0), perspectiveTransform);
					} completion:^(BOOL finished) {
					   [UIView animateWithDuration:0.1 animations:^{
						   self.layer.transform = CATransform3DConcat(CATransform3DMakeRotation(0.0 * M_PI / 180.0, 1.0, 0.0, 0.0), perspectiveTransform);
					   } completion:^(BOOL finished) {
						   if (completion)
							   completion();
					   }];
			...
			[UIView animateWithDuration:0.08 animations:^{
				self.layer.transform = CATransform3DConcat(CATransform3DMakeRotation(-10.0 * M_PI / 180.0, 1.0, 0.0, 0.0), perspectiveTransform);
			} completion:^(BOOL finished) {
